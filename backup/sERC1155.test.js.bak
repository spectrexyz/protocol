  ...
  describe("⇛ constructor", () => {
    describe("» sERC20 base address is not the zero address", () => {
      ...
      it("# it sets sERC20 base address ", async () => {
        expect(await this.sERC1155.sERC20Base()).to.equal(
          this.contracts.sERC20Base.address
        );
      });

      it("# it sets unavailableURI ", async () => {
        expect(await this.sERC1155.unavailableURI()).to.equal(
          this.params.sERC1155.unavailableURI
        );
      });

      it("# it sets unlockedURI", async () => {
        expect(await this.sERC1155.unlockedURI()).to.equal(
      ...
      it("# it sets up admin permissions", async () => {
        expect(
          await this.sERC1155.hasRole(
            this.constants.sERC1155.ADMIN_ROLE,
            this.signers.sERC1155.admin.address
          )
        ).to.equal(true);
        expect(
          await this.sERC1155.getRoleAdmin(this.constants.sERC1155.ADMIN_ROLE)
        ).to.equal(this.constants.sERC1155.ADMIN_ROLE);
      });
    });
    ...
    describe("» sERC20 base address is the zero address", () => {
      it("it reverts", async () => {
        await expect(
          waffle.deployContract(
            this.signers.sERC1155.admin,
            this.sERC1155.artifact,
            [
  ...
  describe("⇛ ERC1155", () => {
    describe("# balanceOf", () => {
      describe("» the queried address is not the zero address", () => {
        describe("» and the queried token type exists", () => {
          ...
          it("it returns the amount of tokens owned by the queried address", async () => {
            ...
            expect(
              await this.sERC1155.balanceOf(
                this.signers.holders[2].address,
                this.data.id
              )
            ).to.equal(0);
          });
      ...
      describe("» the queried address is the zero address", () => {
        ...
        it("it reverts", async () => {
          await expect(
            this.sERC1155.balanceOf(ethers.constants.AddressZero, this.data.id)
    ...
    describe("# balanceOfBatch", () => {
      describe("» input arrays match", () => {
        ...
        describe("» but one of the queried address is the zero address", () => {
          ...
          it("it reverts", async () => {
            await expect(
              this.sERC1155.balanceOfBatch(
                [this.signers.holders[0].address, ethers.constants.AddressZero],
    ...
    describe("# setApprovalForAll", () => {
      describe("» operator is not setting approval status for self", () => {
        ...
        it("it registers approval status", async () => {
          expect(
            await this.sERC1155.isApprovedForAll(
              this.signers.holders[0].address,
      ...
      describe("» operator is setting approval status for self", () => {
        ...
        it("it reverts", async () => {
          await expect(
            this.sERC1155.setApprovalForAll({
              from: this.signers.sERC1155.operator,
              operator: this.signers.sERC1155.operator,
            })
          ).to.be.revertedWith("sERC1155: setting approval status for self");
        });
      });
    });
            ...
            describe("» and the receiver is a contract", () => {
              describe("» and the receiver contract implements onERC1155Received", () => {
                ...
                describe("» but the receiver contract returns an invalid value", () => {
                  ...
                  it("it reverts", async () => {
                    await expect(
                      this.sERC1155.safeTransferFrom({
                        to: this.contracts.ERC1155Receiver,
                        data: "0x12345678",
                      })
                    ).to.be.revertedWith(
    ...
    describe("# safeBatchTransferFrom", () => {
                ...
                describe("» and the receiver contract implements onERC1155Received", () => {
                  describe("» and the receiver contract returns a valid value", () => {
                    ...
                    it("it calls onERC1155Received", async () => {
                      await expect(this.data.tx)
                        .to.emit(
                          this.contracts.ERC1155Receiver,
                          "BatchReceived"
                        )
                        .withArgs(
                          this.signers.holders[0].address,
                  ...
                  describe("» but the receiver contract returns an invalid value", () => {
                    ...
                    it("it reverts", async () => {
                      await expect(
                        this.sERC1155.safeBatchTransferFrom({
                          to: this.contracts.ERC1155Receiver,
                          data: "0x12345678",
                        })
                      ).to.be.revertedWith(
      ...
      describe("» input arrays do not match", () => {
        ...
        it("it reverts", async () => {
          await expect(
            this.sERC1155.safeBatchTransferFrom({
              amounts: [this.constants.amount1],
            })
          ).to.be.revertedWith("sERC1155: ids and amounts length mismatch");
        });
      });
    });
  });
  ...
  describe("⇛ ERC721Receiver", () => {
    describe("# onERC721Received", () => {
      describe("» is called by a standard-compliant ERC721", () => {
        describe("» and spectralization data have a valid length", () => {
          ...
          describe("» but spectralization data does not end up with Derrida magic value", () => {
            ...
            it("it reverts", async () => {
              await expect(
                this.sERC1155.spectralize({
                  transfer: true,
                  derrida: ethers.constants.HashZero,
                })
              ).to.be.revertedWith("sERC1155: invalid spectralization data");
            });
          });
        });
        ...
        describe("» but spectralization data does not have a valid length", () => {
          ...
          it("it reverts", async () => {
            await expect(
              this.sERC1155.spectralize({ transfer: true, short: true })
            ).to.be.revertedWith(
              "sERC1155: invalid spectralization data length"
            );
          });
        });
      });
      ...
      describe("» is called by a non-compliant ERC721", () => {
        ...
        it("it reverts", async () => {
          await expect(
            this.sERC1155.onERC721Received(
  ...
  describe("⇛ sERC1155", () => {
    describe("# spectralize", () => {
      describe("» NFT has never been spectralized", () => {
        describe("» and NFT is ERC721-compliant", () => {
          describe("» and sERC1155 has been approved to transfer NFT", () => {
            ...
            describe("» but NFT is owned by sERC1155", () => {
              ...
              it("it reverts", async () => {
                await expect(this.sERC1155.spectralize()).to.be.revertedWith(
                  "sERC1155: NFT is already owned by sERC1155"
                );
              });
            });
          });
        ...
        describe("» but NFT is not ERC721-compliant", () => {
          ...
          it("it reverts", async () => {
            await expect(
              this.sERC1155.spectralize({ collection: this.signers.others[0] })
            ).to.be.revertedWith("sERC1155: NFT is not ERC721-compliant");
          });
        });
      });
      ...
      describe("» NFT has already been spectralized", () => {
        ...
        describe("» but NFT still is locked", () => {
          ...
          it("it reverts", async () => {
            await expect(this.sERC1155.spectralize()).to.be.revertedWith(
              "sERC1155: NFT is already locked"
            );
          });
        });
      });
      ...
      describe("» spectre exists", () => {
        describe("» and spectre is locked", () => {
          ...
          describe("» but caller is not spectre's guardian", () => {
            ...
            it("it reverts", async () => {
              await expect(
                this.sERC1155.unlock({ from: this.signers.others[0] })
              ).to.be.revertedWith("sERC1155: must be guardian to unlock");
            });
          });
        });
        ...
        describe("» but spectre is not locked anymore", () => {
          ...
          it("it reverts", async () => {
            await expect(this.sERC1155.unlock()).to.be.revertedWith(
              "sERC1155: spectre is not locked"
            );
          });
        });
      });
    ...
    describe("# unlock [by address]", () => {
      describe("» spectre exists", () => {
        ...
        describe("» but spectre is not locked anymore", () => {
          ...
          it("it reverts", async () => {
            await expect(
              this.sERC1155.unlock({ byAddress: true })
            ).to.be.revertedWith("sERC1155: spectre is not locked");
          });
        });
      });
    ...
    describe("# updateUnavailableURI", () => {
      ...
      describe("» caller does not have ADMIN_ROLE", () => {
        ...
        it("it reverts", async () => {
          await expect(
            this.sERC1155.updateUnavailableURI("ipfs://testunavailableURI", {
              from: this.signers.others[0],
            })
          ).to.be.revertedWith(
    ...
    describe("# updateUnlockedURI", () => {
      ...
      describe("» caller does not have ADMIN_ROLE", () => {
        ...
        it("it reverts", async () => {
          await expect(
            this.sERC1155.updateUnlockedURI("ipfs://testunlockedURI", {
              from: this.signers.others[0],
            })
          ).to.be.revertedWith(
        ...
        describe("» and sERC20s are transferred", () => {
          ...
          describe("» and the receiver is a contract", () => {
            ...
            describe("» but the receiver contract returns an invalid value", () => {
              ...
              it("it reverts", async () => {
                await expect(
                  this.sERC20.transfer({ to: this.contracts.ERC1155Receiver })
                ).to.be.revertedWith(
                  "sERC1155: ERC1155Receiver rejected tokens"
                );
              });
