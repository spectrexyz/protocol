...
describe("sERC20Splitter", () => {
  ...
  describe("# constructor", () => {
    ...
    it("# it sets up permissions", async () => {
      expect(
        await this.sERC20Splitter.hasRole(
          await this.sERC20Splitter.DEFAULT_ADMIN_ROLE(),
          this.signers.sERC20Splitter.admin.address
        )
      ).to.equal(true);
      expect(
        await this.sERC20Splitter.hasRole(
          await this.sERC20Splitter.REGISTRAR_ROLE(),
          this.signers.sERC20Splitter.registrar.address
        )
      ).to.equal(true);
    });
  });
  ...
  describe("# register", () => {
    describe("» sender has REGISTRAR_ROLE", () => {
            ...
            describe("» and no share is worth zero", () => {
              describe("» and shares add up to 100%", () => {
                ...
                it("it registers split", async () => {
                  const split = await this.sERC20Splitter.splitOf(
                    this.sERC20.contract.address
                  );

                  expect(
                    await this.sERC20Splitter.isRegistered(
                      this.sERC20.contract.address
                    )
                  ).to.equal(true);
                ...
                it("it emits a Register event", async () => {
                  await expect(this.data.tx)
                    ...
                    .withArgs(
                      this.sERC20.contract.address,
                      this.signers.sERC20Splitter.beneficiaries.map(
                ...
                it("it reverts", async () => {
                  await expect(
                    this.sERC20Splitter.register({
                      shares: [
                        this.params.sERC20Splitter.shares[0],
                        this.params.sERC20Splitter.shares[1],
                        "10",
                      ],
                    })
                  ).to.be.revertedWith(
          ...
          describe("» but one beneficiary is the zero address", () => {
            ...
            it("it reverts", async () => {
              await expect(
                ...
                "SERC20Splitter: beneficiary cannot be the zero address"
              );
            });
          });
        });
      ...
      describe("» but sERC20 split is already registered", () => {
        ...
        it("it reverts", async () => {
          await expect(this.sERC20Splitter.register()).to.be.revertedWith(
            "SERC20Splitter: sERC20 split already registered"
          );
        });
      });
    });
    ...
    describe("» sender does not have REGISTRAR_ROLE", () => {
      ...
      it("it reverts", async () => {
        await expect(
          this.sERC20Splitter.register({ from: this.signers.others[0] })
        ).to.be.revertedWith(
          "SERC20Splitter: must have register role to register"
        );
      });
    });
  });
    ...
    describe("» sERC20 split is registered", () => {
      describe("» and there is something to withdraw", () => {
        ...
        it("it updates split history", async () => {
          ...
          const split1 = await this.sERC20Splitter.splitOf(
            this.sERC20.contract.address
          );
          ...
          const split2 = await this.sERC20Splitter.splitOf(
            this.contracts.sERC20.address
          );
          expect(split2.received).to.equal(1000);
          expect(split2.totalWithdrawn).to.equal(300);
          expect(
            await this.sERC20Splitter.withdrawnBy(
              this.contracts.sERC20.address,
              this.signers.sERC20Splitter.beneficiaries[0].address
            )
          ).to.equal(300);

          await this.sERC20.transfer({ amount: "5000" });
          const split3 = await this.sERC20Splitter.splitOf(
            this.contracts.sERC20.address
          );
          ...
          await this.sERC20Splitter.withdraw({
            from: this.signers.sERC20Splitter.beneficiaries[1],
          });
          const split4 = await this.sERC20Splitter.splitOf(
            this.contracts.sERC20.address
          );
          ...
          expect(
            await this.sERC20Splitter.withdrawnBy(
              this.contracts.sERC20.address,
              this.signers.sERC20Splitter.beneficiaries[1].address
            )
          ).to.equal(600);

          await this.sERC20.transfer({ amount: "6000" });
          const split5 = await this.sERC20Splitter.splitOf(
            this.contracts.sERC20.address
          );
          ...
          await this.sERC20Splitter.withdraw({
            from: this.signers.sERC20Splitter.beneficiaries[0],
          });
          await this.sERC20Splitter.withdraw({
            from: this.signers.sERC20Splitter.beneficiaries[1],
          });
          await this.sERC20Splitter.withdraw({
            from: this.signers.sERC20Splitter.beneficiaries[2],
          });
          const split6 = await this.sERC20Splitter.splitOf(
            this.contracts.sERC20.address
          );
          ...
          expect(
            await this.sERC20Splitter.withdrawnBy(
              this.contracts.sERC20.address,
              this.signers.sERC20Splitter.beneficiaries[2].address
            )
          ).to.equal(7200);
        });
        ...
        it("it transfers sERC20s", async () => {
          expect(
            await this.contracts.sERC20.balanceOf(
              this.signers.sERC20Splitter.beneficiaries[0].address
            )
          ).to.equal(3600);
          expect(
      ...
      describe("» but there is nothing to withdraw", () => {
        ...
        it("it reverts", async () => {
          await expect(this.sERC20Splitter.withdraw()).to.be.revertedWith(
            "SERC20Splitter: nothing to withdraw"
          );
        });
      });
    });
  ...
  describe("# withdrawBatch", () => {
    describe("» all sERC20s splits are registered", () => {
      describe("» and there is something to withdraw for all sERC20s", () => {
        ...
        it("it updates split history", async () => {
          const split1 = await this.sERC20Splitter.splitOf(
            this.data.sERC201.address
          );
          const split2 = await this.sERC20Splitter.splitOf(
            this.data.sERC202.address
          );
        ...
        it("it transfers sERC20s", async () => {
          expect(
            await this.data.sERC201.balanceOf(
              this.signers.sERC20Splitter.beneficiaries[0].address
            )
          ).to.equal(300);
          expect(
            await this.data.sERC202.balanceOf(
              this.signers.sERC20Splitter.beneficiaries[0].address
            )
          ).to.equal(600);
        });
      ...
      describe("» but there is nothing to withdraw for one sERC20", () => {
        ...
        it("it reverts", async () => {
          await expect(this.sERC20Splitter.withdrawBatch()).to.be.revertedWith(
            "SERC20Splitter: nothing to withdraw"
          );
        });
      });
