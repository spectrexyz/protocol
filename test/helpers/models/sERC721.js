...
class sERC721 {
  ...
  static async deploy(ctx) {
    ctx.contracts.sERC721 = await waffle.deployContract(
      ctx.signers.root,
      _sERC721_,
      [ctx.params.sERC721.name, ctx.params.sERC721.symbol]
    );
  ...
  async mint(opts = {}) {
    ...
    this.ctx.data.tx = await this.contract.mint(
      this.ctx.signers.sERC721.owners[0].address,
      this.ctx.params.sERC721.tokenURI
    );
    this.ctx.data.receipt = await this.ctx.data.tx.wait();
    this.ctx.data.tokenId =
      this.ctx.data.receipt.events[0].args.tokenId.toString();
  ...
  async transfer(opts = {}) {
    ...
    this.ctx.data.tx = await this.contract
      .connect(opts.from)
      .transferFrom(opts.from.address, opts.to.address, this.ctx.data.tokenId);
    this.ctx.data.receipt = await this.ctx.data.tx.wait();
  }
  ...
  async approve(opts = {}) {
    ...
    this.ctx.data.tx = await this.ctx.contracts.sERC721
      .connect(opts.from)
      .approve(this.ctx.contracts.sERC1155.address, this.ctx.data.tokenId);
    this.ctx.data.receipt = await this.ctx.data.tx.wait();
  }
