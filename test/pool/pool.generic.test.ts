...
import { actionId } from "../helpers/balancer/v2-helpers/src/models/misc/actions";
import { BigNumberish, fp } from "../helpers/balancer/v2-helpers/src/numbers";
import {
  MAX_INT22,
  MAX_UINT10,
  MAX_UINT31,
  MAX_UINT64,
  MIN_INT22,
} from "../helpers/balancer/v2-helpers/src/constants";
import {
  MINUTE,
  advanceTime,
  currentTimestamp,
  lastBlockNumber,
} from "../helpers/balancer/v2-helpers/src/time";

import TokenList from "../helpers/balancer/v2-helpers/src/models/tokens/TokenList";
import WeightedPool from "../helpers/balancer/v2-helpers/src/models/pools/weighted/WeightedPool";
import {
  Sample,
  WeightedPoolType,
} from "../helpers/balancer/v2-helpers/src/models/pools/weighted/types";

import { itBehavesAsWeightedPool } from "./BaseWeightedPool.behavior";

require("../helpers/balancer/v2-common/setupTests");
...
describe("WeightedPool2Tokens", function () {
  ...
  sharedBeforeEach("deploy pool", async () => {
    const params = {
      poolType: WeightedPoolType.WEIGHTED_POOL_2TOKENS,
      tokens,
      weights,
      owner,
    };
  ...
  describe("oracle", () => {
    ...
    ) => Promise<{ receipt: ContractReceipt }>;
    ...
    const itUpdatesTheOracleData = (
      action: PoolHook,
      lastChangeBlockOffset = 0
    ) => {
      context("without updated oracle", () => {
        it("updates the oracle data", async () => {
          ...
          expect(currentMiscData.oracleIndex).to.equal(
            previousData.oracleIndex.add(1)
          );
      ...
      context("with updated oracle", () => {
        ...
        it("stores the pre-action spot price", async () => {
          const expectedSpotPrice = await pool.estimateSpotPrice(
            previousBalances
          );
          const actual = await pool.instance.fromLowResLog(
            newSample.logPairPrice
          );

          expect(actual).to.equalWithError(
            expectedSpotPrice,
            MAX_RELATIVE_ERROR
          );
        });
        ...
        it("stores the pre-action BPT price", async () => {
          const expectedBPTPrice = await pool.estimateBptPrice(
            0,
            previousBalances[0],
            previousTotalSupply
          );
          const actual = await pool.instance.fromLowResLog(
            newSample.logBptPrice
          );
        ...
        it("stores the pre-action invariant", async () => {
          const expectedInvariant = await pool.estimateInvariant(
            previousBalances
          );
          const actual = await pool.instance.fromLowResLog(
            newSample.logInvariant
          );
    ...
    const itDoesNotUpdateTheOracleData = (
      action: PoolHook,
      lastChangeBlockOffset = 0
    ) => {
    ...
    const itCachesTheLogInvariantAndSupply = (
      action: PoolHook,
      lastChangeBlockOffset = 0
    ) => {
      it("caches the log of the last invariant", async () => {
        ...
        const currentMiscData = await pool.getMiscData();
        const actualInvariant = await pool.instance.fromLowResLog(
          currentMiscData.logInvariant
        );
        const expectedInvariant = await pool.getLastInvariant();
        expect(actualInvariant).to.be.equalWithError(
          expectedInvariant,
          MAX_RELATIVE_ERROR
        );
      });
      ...
      it("caches the total supply", async () => {
        ...
        const currentMiscData = await pool.getMiscData();
        const actualTotalSupply = await pool.instance.fromLowResLog(
          currentMiscData.logTotalSupply
        );
        const expectedTotalSupply = await pool.totalSupply();
        expect(actualTotalSupply).to.equalWithError(
          expectedTotalSupply,
          MAX_RELATIVE_ERROR
        );
      });
    ...
    const itDoesNotCacheTheLogInvariantAndSupply = (
      action: PoolHook,
      lastChangeBlockOffset = 0
    ) => {
      it("does not cache the log invariant and supply", async () => {
        ...
        const currentMiscData = await pool.getMiscData();
        expect(currentMiscData.logInvariant).to.equal(
          previousMiscData.logInvariant
        );
        expect(currentMiscData.logTotalSupply).to.equal(
          previousMiscData.logTotalSupply
        );
      });
    };

    const itDoesNotDoAnythingWhenDisabled = (
      action: PoolHook,
      lastChangeBlockOffset = 0
    ) => {
    ...
    describe("join", () => {
      const action = (lastChangeBlock: number) =>
        pool.joinGivenIn({ amountsIn: fp(1), lastChangeBlock });
    ...
    describe("exit", () => {
      const action = async (lastChangeBlock: number) => {
        ...
        return await pool.multiExitGivenIn({
          bptIn: balance.div(2),
          lastChangeBlock,
          from: lp,
        });
      ...
      context("when the pool is paused", () => {
        ...
        context("when the latest change block is an old block", () => {
          ...
          context("when the oracle is enabled", () => {
            itDoesNotUpdateTheOracleData(action, lastChangeBlockOffset);
            itDoesNotCacheTheLogInvariantAndSupply(
              action,
              lastChangeBlockOffset
            );
          });
    ...
    describe("swaps", () => {
      ...
      context("given in", () => {
        const action = (lastChangeBlock: number) =>
          pool.swapGivenIn({ in: 0, out: 1, amount, lastChangeBlock });
      ...
      context("given out", () => {
        const action = (lastChangeBlock: number) =>
          pool.swapGivenOut({ in: 1, out: 0, amount, lastChangeBlock });
      ...
      context("when it starts disabled", () => {
        ...
        context("when the pool was initialized", async () => {
          ...
          it("can only be updated by the admin", async () => {
            await expect(pool.enableOracle({ from: other })).to.be.revertedWith(
              "SENDER_NOT_ALLOWED"
            );
            await expect(pool.enableOracle({ from: owner })).to.be.revertedWith(
              "SENDER_NOT_ALLOWED"
            );
          });
  ...
  describe("misc data", () => {
    ...
    const assertPacking = async (
      ...
      expect(miscData.oracleSampleCreationTimestamp).to.be.equal(
        oracleSampleCreationTimestamp
      );
