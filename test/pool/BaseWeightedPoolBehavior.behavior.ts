...
import { PoolSpecialization, SwapKind } from "../helpers/balancer/balancer-js";
import {
  BigNumberish,
  bn,
  fp,
  pct,
} from "../helpers/balancer/v2-helpers/src/numbers";
import { ZERO_ADDRESS } from "../helpers/balancer/v2-helpers/src/constants";
...
export function itBehavesAsWeightedPool(
  ...
  sharedBeforeEach("deploy tokens", async () => {
    allTokens = await TokenList.create(["MKR", "DAI", "SNX", "BAT"], {
      sorted: true,
    });
    ...
    context("when the creation fails", () => {
      if (poolType != WeightedPoolType.WEIGHTED_POOL_2TOKENS) {
        it("reverts if the number of tokens and weights do not match", async () => {
          ...
          await expect(deployPool({ weights: badWeights })).to.be.revertedWith(
  ...
  describe("onJoinPool", () => {
    ...
    it("fails if caller is not the vault", async () => {
      await expect(
        pool.instance
          .connect(lp)
          .onJoinPool(pool.poolId, lp.address, other.address, [0], 0, 0, "0x")
    ...
    it("fails if wrong user data", async () => {
      ...
      await expect(pool.join({ data: wrongUserData })).to.be.revertedWith(
        "Transaction reverted without a reason"
      );
    });
    ...
    context("initialization", () => {
      it("grants the n * invariant amount of BPT", async () => {
        ...
        expect(await pool.balanceOf(recipient)).to.be.equalWithError(
      ...
      it("fails if already initialized", async () => {
        ...
        await expect(pool.init({ initialBalances })).to.be.revertedWith(
          "UNHANDLED_JOIN_KIND"
        );
      });
    ...
    context("join exact tokens in for BPT out", () => {
      it("fails if not initialized", async () => {
        await expect(
          pool.joinGivenIn({ recipient, amountsIn: initialBalances })
      ...
      context("once initialized", () => {
        ...
        it("reverts if paused", async () => {
          ...
          await expect(pool.joinGivenIn({ amountsIn })).to.be.revertedWith(
            "PAUSED"
          );
        });
      });
    ...
    context("join token in for exact BPT out", () => {
      ...
      it("fails if not initialized", async () => {
        await expect(pool.joinGivenOut({ bptOut, token })).to.be.revertedWith(
          "UNINITIALIZED"
        );
      });
    ...
    context("join all tokens in for exact BPT out", () => {
      ...
      context("once initialized", () => {
        ...
        it("grants exact BPT for tokens in", async () => {
          ...
          const expectedAmountsIn = initialBalances.map((balance) =>
            balance.div(2)
          );
  ...
  describe("onExitPool", () => {
    ...
    it("fails if wrong user data", async () => {
      ...
      await expect(pool.exit({ data: wrongUserData })).to.be.revertedWith(
        "Transaction reverted without a reason"
      );
    });
    ...
    context("exit exact BPT in for all tokens out", () => {
      ...
      it("does not revert if paused", async () => {
        ...
        const bptIn = previousBptBalance.div(2);
        await expect(pool.multiExitGivenIn({ from: lp, bptIn })).not.to.be
          .reverted;
      });
    });
    ...
    context("exit BPT in for exact tokens out", () => {
      ...
      it("fails if more BTP needed", async () => {
        ...
        await expect(
          pool.exitGivenOut({ from: lp, amountsOut, maximumBptIn })
  ...
  describe("onSwap", () => {
    ...
    context("given in", () => {
      ...
      it("calculates amount out", async () => {
        const amount = fp(0.1);
        const amountWithFees = amount
          .mul(POOL_SWAP_FEE_PERCENTAGE.add(fp(1)))
          .div(fp(1));
      ...
      it("calculates max amount out", async () => {
        ...
        const maxAmountInWithFees = maxAmountIn
          .mul(POOL_SWAP_FEE_PERCENTAGE.add(fp(1)))
          .div(fp(1));
        const expectedAmountOut = await pool.estimateGivenIn({
          in: 1,
      ...
      it("reverts if token in exceeds max in ratio", async () => {
        ...
        await expect(
          pool.swapGivenIn({ in: 1, out: 0, amount })
      ...
      if (poolType != WeightedPoolType.WEIGHTED_POOL_2TOKENS) {
        it("reverts if token in is not in the pool", async () => {
          await expect(
            pool.swapGivenIn({ in: allTokens.BAT, out: 0, amount: 1 })
          ).to.be.revertedWith("INVALID_TOKEN");
        });

        it("reverts if token out is not in the pool", async () => {
          await expect(
            pool.swapGivenIn({ in: 1, out: allTokens.BAT, amount: 1 })
          ).to.be.revertedWith("INVALID_TOKEN");
        });
      }
      ...
      it("reverts if paused", async () => {
        ...
        await expect(
          pool.swapGivenIn({ in: 1, out: 0, amount: 1 })
        ).to.be.revertedWith("PAUSED");
      });
    });
    ...
    context("given out", () => {
      ...
      it("reverts if token in exceeds max out ratio", async () => {
        ...
        await expect(
          pool.swapGivenOut({ in: 1, out: 0, amount })
      ...
      if (poolType != WeightedPoolType.WEIGHTED_POOL_2TOKENS) {
        it("reverts if token in is not in the pool when given out", async () => {
          await expect(
            pool.swapGivenOut({ in: allTokens.BAT, out: 0, amount: 1 })
          ).to.be.revertedWith("INVALID_TOKEN");
        });

        it("reverts if token out is not in the pool", async () => {
          await expect(
            pool.swapGivenOut({ in: 1, out: allTokens.BAT, amount: 1 })
          ).to.be.revertedWith("INVALID_TOKEN");
        });
      }
      ...
      it("reverts if paused", async () => {
        ...
        await expect(
          pool.swapGivenOut({ in: 1, out: 0, amount: 1 })
        ).to.be.revertedWith("PAUSED");
      });
    });
  });
  ...
  describe("protocol swap fees", () => {
    ...
    context("with previous swap", () => {
      ...
      sharedBeforeEach("compute expected due protocol fees", async () => {
        ...
        expectedDueProtocolFeeAmounts = ZEROS.map((n, i) =>
          i === paidTokenIndex ? protocolFeeAmount : n
        );
      });
      ...
      it("pays swap protocol fees on join exact tokens in for BPT out", async () => {
        ...
        expect(result.dueProtocolFeeAmounts).to.be.equalWithError(
    ...
    context("with swap and exceeded min invariant ratio", () => {
      ...
      sharedBeforeEach("compute expected due protocol fees", async () => {
        const paidTokenIndex = pool.weights.indexOf(pool.maxWeight);
        const feeAmount = await pool.estimateMaxSwapFeeAmount(
          paidTokenIndex,
          protocolFeePercentage,
          currentBalances
        );
        expectedDueProtocolFeeAmounts = ZEROS.map((n, i) =>
          i === paidTokenIndex ? feeAmount : n
        );
      });
