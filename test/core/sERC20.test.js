...
describe("sERC20", () => {
  ...
  describe("⇛ initialize", () => {
      ...
      describe("» and cap is not zero", () => {
        describe("» and admin is not the zero address", () => {
          ...
          it("# it sets sERC20's symbol", async () => {
            expect(await this.sERC20.symbol()).to.equal(
              this.params.sERC20.symbol
            );
          });
          ...
          it("# it sets sERC20's admin", async () => {
            expect(
              await this.sERC20.hasRole(
                await this.contracts.sERC20.DEFAULT_ADMIN_ROLE(),
                this.signers.sERC20.admin.address
              )
            ).to.equal(true);
          });

          it("# it sets sERC20's pegged sERC1155's address", async () => {
            expect(await this.sERC20.sERC1155()).to.equal(
              this.sERC1155.contract.address
            );
          });
        });
    ...
    describe("» is called on the implementation contract", () => {
      ...
      it("it reverts", async () => {
        await expect(this.sERC20.initialize()).to.be.revertedWith(
  ...
  describe("# approve", () => {
    describe("» spender is not the zero address", () => {
      before(async () => {
        ...
        await this.sERC20.approve({
          spender: this.signers.others[0],
          amount: this.params.sERC20.amount,
        });
      });

      it("it updates spender's allowance", async () => {
        expect(
          await this.sERC20.allowance(
            this.signers.holders[0].address,
            this.signers.others[0].address
          )
        ).to.equal(this.params.sERC20.amount);
      });
    ...
    describe("» spender is the zero address", () => {
      ...
      it("it reverts", async () => {
        await expect(
          this.sERC20.approve({
            spender: { address: ethers.constants.AddressZero },
          })
    ...
    describe("» recipient is not the zero address", () => {
      describe("» and sender has enough balance", () => {
        ...
        it("it transfers tokens", async () => {
          expect(await this.sERC20.balanceOf(this.signers.holders[0])).to.equal(
            this.params.sERC20.balance.sub(this.params.sERC20.amount)
          );
          expect(await this.sERC20.balanceOf(this.signers.holders[1])).to.equal(
            this.params.sERC20.amount
          );
        });
      ...
      describe("» but sender does not have enough balance", () => {
        ...
        it("it reverts", async () => {
          await expect(
            this.sERC20.transfer({
              to: this.signers.holders[1],
              amount: this.params.sERC20.balance.add(
      ...
      describe("» and recipient is not the zero address", () => {
        describe("» and spender has enough approved balance", () => {
          describe("» and token owner has enough balance", () => {
            ...
            it("it transfers tokens", async () => {
              expect(
                await this.sERC20.balanceOf(this.signers.holders[0])
              ).to.equal(
                this.params.sERC20.balance.sub(this.params.sERC20.amount)
              );
              expect(
                await this.sERC20.balanceOf(this.signers.holders[1])
              ).to.equal(this.params.sERC20.amount);
            });
        ...
        describe("» but spender does not have enough approved balance", () => {
          ...
          it("it reverts", async () => {
            await expect(this.sERC20.transferFrom()).to.be.revertedWith(
  ...
  describe("# setRoleAdmin", () => {
    describe("» caller has admin role", () => {
      before(async () => {
        await setup(this);
        await this.sERC20.setRoleAdmin({
          adminRole: ethers.BigNumber.from(
            "0xa49807205ce4d355092ef5a8a18f56e8913cf4a201fbe287825b095693c2177"
          ),
        });
      });
      ...
      it("it sets admin role", async () => {
        expect(
          await this.sERC20.getRoleAdmin(this.constants.sERC20.MINT_ROLE)
        ).to.equal(
    ...
    describe("» caller does not have admin role", () => {
      ...
      it("it reverts", async () => {
        await expect(
          this.sERC20.setRoleAdmin({ from: this.signers.others[0] })
        ).to.be.revertedWith("sERC20: must be role admin to set role admin");
      });
    });
  });
  ...
  describe("# burn", () => {
    describe("» burnt amount is inferior to balance", () => {
      describe("» and sERC20 is not paused", () => {
        ...
        it("it burns tokens", async () => {
          expect(await this.sERC20.totalSupply()).to.equal(
            this.params.sERC20.balance.sub(this.params.sERC20.amount)
          );
          expect(await this.sERC20.balanceOf(this.signers.holders[0])).to.equal(
            this.params.sERC20.balance.sub(this.params.sERC20.amount)
      ...
      describe("» but sERC20 is paused", () => {
        ...
        it("it reverts", async () => {
          await expect(this.sERC20.burn()).to.be.revertedWith(
    ...
    describe("» burnt amount is superior to balance", () => {
      ...
      it("it reverts", async () => {
        await expect(
          this.sERC20.burn({
            amount: this.params.sERC20.balance.add(ethers.BigNumber.from("1")),
          })
  ...
  describe("# burnFrom", () => {
    describe("» and spender has enough approved balance", () => {
      ...
      describe("» but token owner does not have enough balance", () => {
        before(async () => {
          ...
          await this.sERC20.approve({
            spender: this.signers.others[0],
            amount: this.params.sERC20.balance.add(this.params.sERC20.balance),
          });
        });

        it("it reverts", async () => {
          await expect(
            this.sERC20.burnFrom({
              amount: this.params.sERC20.balance.add(
                this.params.sERC20.balance
              ),
            })
    ...
    describe("» but spender does not have enough approved balance", () => {
      ...
      it("it reverts", async () => {
        await expect(this.sERC20.burnFrom()).to.be.revertedWith(
  ...
  describe("# mint", () => {
    describe("» caller has MINT_ROLE", () => {
      describe("» cap is not reached yet", () => {
        describe("» and minted amount is inferior to cap", () => {
          describe("» and sERC20 is not paused", () => {
            ...
            it("it mints tokens", async () => {
              ...
              expect(
                await this.sERC20.balanceOf(this.signers.holders[0])
              ).to.equal(this.params.sERC20.balance);
            });
          ...
          describe("» but sERC20 is paused", () => {
            ...
            it("it reverts", async () => {
              await expect(this.sERC20.mint()).to.be.revertedWith(
                "ERC20Pausable: token transfer while paused"
              );
            });
          });
        ...
        describe("» but amount is superior to cap", () => {
          ...
          it("it reverts", async () => {
            await expect(
              this.sERC20.mint({
                amount: this.params.sERC20.cap.add(ethers.BigNumber.from("1")),
              })
      ...
      describe("» cap is already reached", () => {
        ...
        it("it reverts", async () => {
          await expect(this.sERC20.mint({ amount: 1 })).to.be.revertedWith(
    ...
    describe("» caller does not have MINT_ROLE", () => {
      ...
      it("it reverts", async () => {
        await expect(
          this.sERC20.mint({ from: this.signers.others[0] })
        ).to.be.revertedWith("sERC20: must have MINT_ROLE to mint");
      });
    });
  });
  ...
  describe("# pause", () => {
    ...
    describe("# caller does not have PAUSE_ROLE", () => {
      ...
      it("it reverts", async () => {
        await expect(
          this.sERC20.pause({ from: this.signers.others[0] })
        ).to.be.revertedWith("sERC20: must have PAUSE_ROLE to pause");
      });
    });
  });
  ...
  describe("# unpause", () => {
    ...
    describe("# caller does not have PAUSE_ROLE", () => {
      ...
      it("it reverts", async () => {
        await expect(
          this.sERC20.unpause({ from: this.signers.others[0] })
        ).to.be.revertedWith("sERC20: must have PAUSE_ROLE to unpause");
      });
    });
  });
  ...
  describe("# snapshot", () => {
    describe("» caller has SNAPSHOT_ROLE", () => {
      ...
      it("it snapshots sERC20's total supply", async () => {
        expect(await this.sERC20.totalSupplyAt(this.data.snapshotId)).to.equal(
          this.params.sERC20.balance
        );
      });
      ...
      it("it snapshots sERC20's balances", async () => {
        expect(
          await this.sERC20.balanceOfAt(
            this.signers.holders[0].address,
            this.data.snapshotId
          )
        ).to.equal(this.params.sERC20.balance.sub(this.params.sERC20.amount));
      });
    ...
    describe("» caller does not have SNAPSHOT_ROLE", () => {
      ...
      it("it reverts", async () => {
        await expect(
          this.sERC20.snapshot({ from: this.signers.others[0] })
        ).to.be.revertedWith("sERC20: must have SNAPSHOT_ROLE to snapshot");
      });
    });
  });
});
