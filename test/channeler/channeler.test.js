...
describe("Channeler", () => {
  ...
  describe("# fractionalize", () => {
    describe("» caller is NFT owner", () => {
      describe("» and channeler is not paused", () => {
        before(async () => {
          await setup.channeler(this);
      ...
      describe("» but channeler is paused", () => {
        before(async () => {
          await setup.channeler(this);
          await this.channeler.pause();
        });
    ...
    describe("» caller is not NFT owner", () => {
      before(async () => {
        await setup.channeler(this);
      });

      it("it reverts", async () => {
        await expect(this.channeler.fractionalize({ from: this.signers.others[0] })).to.be.revertedWith("Channeler: must be NFT owner to fractionalize");
      });
    });
  });
